...
export default function ListaTurnos({ turnos, loading, onSubstituir, onFalta, onEditar, onEliminar }: ListaTurnosProps) {
  ...
  // Limpar estado de eliminação se a lista mudar
  useEffect(() => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {turnos.map((turno) => {
        // Filtrar apenas as necessidades ativas (true)
        const activeNeeds = Object.entries(turno.care_needs || {})
        ...
        return (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              {/* LADO ESQUERDO: Info Cliente e Turno */}
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  {/* Horário e Local */}
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-slate-500 mb-2">
                  ...
                  {/* NOVO: Etiquetas das Necessidades do Cliente */}
                  {activeNeeds.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {activeNeeds.map(need => (
                        <span key={need} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-wide border border-slate-200">
                           {need}
                        </span>
                      ))}
                    </div>
              ...
              {/* LADO DIREITO: Cuidadora e Botões */}
                  ...
                  <div className="flex items-center gap-1">
                      ...
                      <button 
                        onClick={() => {
                          ...
                          setTimeout(() => onEliminar(turno.id), 500); 
                        }}
